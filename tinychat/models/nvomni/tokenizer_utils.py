...
from .constants import IGNORE_INDEX, SENTINEL_TOKEN, MEDIA_TOKENS, MM_BOS_EOS_TOKENS
...
def tokenize_conversation(
    ...
    # add multi-modal bos eos tokens to text directly
    if mm_use_bos_eos_tokens:

        def add_mm_bos_eos_tokens(text: str) -> str:
            for k, (_bos, _eos) in MM_BOS_EOS_TOKENS.items():
                if unified_audio_encoder and k == "speech":
                    _bos = MM_BOS_EOS_TOKENS["sound"][0]
                    _eos = MM_BOS_EOS_TOKENS["sound"][1]
