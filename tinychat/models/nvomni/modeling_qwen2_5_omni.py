...
class Qwen2_5OmniPreTrainedModelForConditionalGeneration(Qwen2_5OmniPreTrainedModel):
    ...
    def get_llm_pos_ids_for_vision(
        ...
        t_index = (
            torch.Tensor(t_index).to(llm_grid_h.device).view(-1, 1).expand(-1, llm_grid_h * llm_grid_w).flatten().long()
        )
    ...
    def get_rope_index(
        ...
        for i, src_item in enumerate(raw_input_ids):
            ...
            for idx in range(len(src_item)):
                ...
                if src_item[idx] not in [audio_token_id, video_token_id, image_token_id]:
                    ...
                    t_index_split_chunk = self.split_list_into_ranges(t_index, t_ntoken_per_chunk)  # 每个chunk每格对应的时间戳
        ...
        if padding_side is not None:
            ...
            attention_mask = self._pad_to_list_of_tensors_1d(attention_mask, padding_value=0, padding_side=padding_side)
...
class Qwen2_5OmniAudioEncoderLayer(nn.Module):
    ...
    def forward(
        ...
        # Variable-length sequence handling: cu_seqlens is used to efficiently process batches where different sequences have different lengths, avoiding the need for padding.
        # Attention mask creation: In the audio attention implementation, cu_seqlens is used to create attention masks that ensure tokens only attend to valid positions within their own sequence.
        # Here, each window covers only 100 tokens in qwen2.5 omni
...
class Qwen2_5OmniAudioEncoder(Qwen2_5OmniPreTrainedModel):
    ...
    def forward(
        ...
        for index_ in range(feature_lens.shape[0]):
            ...
            # Split the audio features into chunks of size n_window * 2
            ...
            # Check if the remaining feature length is valid
            if not (feature_length <= 0 and feature_length > -2 * self.n_window):
                raise ValueError(f"feature_length: {feature_length}, n_window: {self.n_window}, input_features.shape: {input_features.shape}")
...
class Qwen2_5OmniToken2WavBigVGANModel(Qwen2_5OmniPreTrainedModel):
    ...
    def __init__(self, config: Qwen2_5OmniBigVGANConfig):
        ...
        # post conv
        print(
            f"YE Warning: Hacking SnakeBeta in talker to load official checkpoint. Not sure about the error cause and whether this is proper or not!"
        )
